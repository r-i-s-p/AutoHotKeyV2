...
:*:\not::¬
:*:\all::∀
:*:\iff::↔   ; same as equiv, diff name for diff symbol
:*:\equiv::⇔ ; same as iff, diff name for diff symbol
:*:\xor::⊻
:*:\nand::↑
:*:\odot::⊙
:*:\oplus::⊕
...
:*:\tm::™
:*:\emd::—
::\deg::° ; no asterisk so that it doesn't interfere with \degC
:*:\euro::€
:*:\degf::℉
:*:\degc::℃
:*:\amogus::ඞ
...
:*R:importrange::=IMPORTRANGE("url", "sheet_name!A1:B1")
:*R:vlookup::=VLOOKUP(search_key, range, column_no, FALSE)
...
; Substitute UK English to US English
::aeroplane::airplane
::artefact::artifact
::British::American
::cheque::check
...
;=================================================================================================================================================================
; NOTE: Compound nouns are usually written as one word, compound verbs are generally written as two, and compound adjectives are very often written with a hyphen. 
; See https://www.merriam-webster.com/help/faq-compound-words
;=================================================================================================================================================================
; Compound words
::auto correct::autocorrect
::auto capitalize::autocapitalize
::ice-cream::ice cream
::nonexperimental::non-experimental 
::on line::online
::web cam::webcam
::web page::webpage
::web site::website

; ########## Improvements: Diction (custom) ##########
::a lot of::many of 
...
::reduce the effect::impair
::reduces the effect::impairs
...
:*:\pff::\frac{{}\partial {}}{{}\partial {}}{left 12}

:*:\tx::\text{{}{}}{left 1}

:*:\lr::{\}left( `n{\}right){left 8}
...
:*:\vint::\int_0{^}\infin d{^}3\textbf{{}p{}} d{^}3\textbf{{}q{}}{left 11}

:*:\pmat::\begin{{}pmatrix{}} `n`n\end{{}pmatrix{}}{left 13}

:*:\align::\begin{{}align*{}} `n`n\end{{}align*{}}{left 13}
...
:*:\ddt::\frac{{}d{}}{{}dt{}}{left 5}

:*:\ideal::H = \frac{{}p{^}2{}}{{}2m{}} + U(q)

:*:\thermal::\bigg(\sqrt{{}\frac{{}2\pi m{}}{{}\beta{}}{}}\bigg){^}{{}3N{}}

; ############################## HTML/CSS ##############################

:*:\dropcap::<p class="dropcap">`n</p>{left 5}

::\sub::<sub></sub>{left 6} ; needs escape sequence after hotstring because of `\subset` above
...
::\glob::glob.glob('pathname', *, root_dir=None, dir_fd=None, recursive=True, include_hidden=False)
...
::\plplot::
(
plt.plot(X, Y, 'b', linewidth=2, label='')
...
$^+q::
{
    if WinActive("ahk_exe zotero.exe") {
        ...
        MsgBox("Unused hotkey")
    } 
}

; Placeholder hotkey for temporary functions via {Control}+{Shift}+{R} 
^+r::
{  
  MsgBox("Unused hotkey")  
}
...
$^+t::{                ; "$" prevents the hotkey/function from calling itself
    while GetKeyState("LCtrl", "P") AND GetKeyState("LShift", "P") AND GetKeyState("t", "P")
        sleep 10
    if (A_TimeSinceThisHotkey < 266) AND WinActive("ahk_exe firefox.exe") {
        Send ("^+t")
        return
    }
    else {
...
; Open Merriam Webster in Firefox and search the highlighted text via {Ctrl}+{Shift}+{D}
^+d:: 
{
    ; Preserve clipboard
    oldClip := ClipboardAll()
    ...
    Send("^t")
    Sleep(50)
    Send("https://www.merriam-webster.com/dictionary/")
    Send(query)
    Send("{Enter}")
...
^+z:: {
    ...
    MsgBox("Color at current cursor pos: " PixelGetColor(MouseX, MouseY),, "T1")
    A_Clipboard := PixelGetColor(MouseX, MouseY)
}

; Return the clipboard text replacing spaces with underscores via {Control}+{Shift}+{X} 
^+x::
{
    ; Get the current window's ID
    HWND := WinGetID("A")

    ; Copy text before ApplyTransform function is called (clicking buttons changes focus window; 
    ; cannot highlight text and click buttons for output. Either use {Ctrl}+{C} or the hotkey)
    ;==========================================================
    ; Preserve clipboard
    oldClip := ClipboardAll()
    A_Clipboard := ""  
    ...
    MyGui := Gui("AlwaysOnTop -MaximizeBox +MinSize240x389", "AHK Text Case Editor",)

    ; GUI: Capitalization and Separator options
    ;==========================================================
    MyGui.SetFont("s11 bold", "Arial")
    MyGui.Add("Text", "R1", "Capitalization") 
    MyGui.SetFont("s11 norm", "Arial")
    DDL_capital := MyGui.Add("DropDownList", "Choose1", ["Lowercase", "Uppercase", "Titlecase", "No change"]) 

    MyGui.SetFont("s11 bold", "Arial")
    MyGui.Add("Text", "R1", "Separator") 
    MyGui.SetFont("s11 norm", "Arial")
    DDL_sep := MyGui.Add("DropDownList", "Choose3", ["None", "Kebab-case", "Snake_case", "Default (Space)"]) 
    
    ; GUI: Apply transformation upon selection; show GUI
    ;==========================================================   
    MyGui.SetFont("s10 norm", "Arial")
    btnApply := MyGui.Add("Button", "xs0 y130 WP HP Center", "Apply")
    ...
    ApplyTransform(*) {
        ...
        if (DDL_capital.value = 1) {
            for w in words
                result_cap := result_cap . StrLower(w) . A_space
        }
        else if (DDL_capital.value = 2) {
            for w in words
                result_cap := result_cap . StrUpper(w) . A_space
        }
        else if (DDL_capital.value = 3) {
            for w in words
                result_cap := result_cap . Format("{:T}", w) . A_space 
        }
        else {
            for w in words
        ...
        else if (DDL_sep.value = 2) {
            result_sep := StrReplace(result_sep, ' ', '-')
        }
        else if (DDL_sep.value = 3) {
            result_sep := StrReplace(result_sep, ' ', '_')
        }
        else if (DDL_sep.value = 4) {
...
^+b:: 
{
    if WinActive("ahk_exe firefox.exe") {
        ;======== Prepend "https`://archive.is/newest/" ========
        Send("^l")
        Send("{LCtrl}{Left}")
        Sleep(50)
...
^+n:: 
{
    ; See Notion Developers page for help: https://developers.notion.com/docs/authorization
    ;====================================================================================== 
    ; ======== Personal Notion Information ========
    authToken := "INSERT_TOKEN_HERE"                             
    databaseID := "INSERT_DATABASE_ID_HERE"                                              
    url := "https://api.notion.com/v1/pages"
    
    ; ======== Notion page title ========
    InputBoxObj := InputBox("[n, v…] Word – (context) Meaning1 // Meaning2", "AHK to Notion API", "w314 h100", "word - meaning")

    ; ======== Cancel request if inputbox closed ========
    if InputBoxObj.Result == "Cancel" OR InputBoxObj.Result == "Timeout" {
        return
    }

    ; ======== Format the page title ========
    arrayText := StrSplit(Trim(InputBoxObj.Value), " - ")
    arrayText[1] := StrUpper(SubStr(arrayText[1], 1, 1)) . StrLower(SubStr(arrayText[1], 2))      ; Not Title case – only first letter of a phrase is capitalized
    arrayText[2] := Format("{:L}", arrayText[2])
    notion_page_title := arrayText[1] . " – " . arrayText[2]
    
    ; ======== Create HTTP request with required headers ========
    objNotionapiRequest := ComObject("WinHttp.WinHttpRequest.5.1")
    objNotionapiRequest.Open("POST", url, false)

    objNotionapiRequest.SetRequestHeader("Authorization", "Bearer " authToken)
    objNotionapiRequest.SetRequestHeader("Content-Type", "application/json")
    objNotionapiRequest.SetRequestHeader("Notion-Version", "2025-09-03")
...
; Format substitutes the {} with these variables; one-based indexing
databaseID, notion_page_title
)

    ; ======== Send request; read response ========
    objNotionapiRequest.Send(body)

    status := objNotionapiRequest.Status
    responsetext := objNotionapiRequest.ResponseText

    MsgBox("Status: " Status "`n`nResponse:`n" ResponseText, "Notion API Status and Responsetext", "T2.5")
}

; Open Google Maps in Firefox and search the highlighted text via {Ctrl}+{Shift}+{M}
^+m:: 
{
    ; Preserve clipboard
    oldClip := ClipboardAll()
    ...
    Send("^t")
    Sleep(50)
    Send("https://www.google.com/maps/search/")
    Send(query)
    Send("{Enter}")
...
CapsLock::{            
   while GetKeyState("CapsLock", "P")
      sleep 10
   if (A_TimeSinceThisHotkey >= 266) {
      Send ("{LAlt Down}{CapsLock}{LAlt Up}")
      sleep 10
      
      SetCapsLockState False
      WinActive("ahk_exe Wox.exe")
      WinWaitActive("ahk_exe Wox.exe")
   }
   else
      Exit
}
...
; ########## Mouse Remapping ##########
#HotIf WinActive("ahk_exe Acrobat.exe")  ; mouse button copy-paste remapping exclusive to Acrobat
...
+<!W::{
   sleep 10
   Send ("{LCtrl Down}{Up}{LCtrl Up}")
}

+<!S::{
   sleep 10
   Send ("{LCtrl Down}{Down}{LCtrl Up}")
}
...
; #################################### Autocapitalize ####################################
#HotIf WinActive("ahk_exe Code.exe")  ; autocapitalization disabled in VS code
::i::i
#HotIf                                ; resume global default

; "I" pronoun (custom; global default)
:*C:i ::I{space}
:*C:i'::I'
...
:C:algiers::Algiers
:C:andorra::Andorra
:C:andorra la vella::Andorra la Vella
:C:angola::Angola
:C:luanda::Luanda
:C:antigua and barbuda::Antigua and Barbuda
:C:saint john's::Saint John's
...
:C:brazil::Brazil
:C:brasilia::Brasília
:C:sao paulo::São Paulo
:C:rio de janeiro::Rio de Janeiro
:C:brunei::Brunei
:C:bandar seri begawan::Bandar Seri Begawan
:C:bulgaria::Bulgaria
:C:sofia::Sofia
:C:burkina faso::Burkina Faso
:C:ouagadougou::Ouagadougou
:C:burundi::Burundi
:C:gitega::Gitega
:C:bujumbura::Bujumbura
:C:cambodia::Cambodia
:C:phnom penh::Phnom Penh
:C:cameroon::Cameroon
:C:yaounde::Yaoundé
:C:canada::Canada
:C:ottawa::Ottawa
...
:C:comoros::Comoros
:C:moroni::Moroni
:C:congo (republic of the)::Republic of the Congo
:C:brazzaville::Brazzaville
:C:congo (democratic republic of the)::Democratic Republic of the Congo
:C:kinshasa::Kinshasa
...
:C:prague::Prague
:C:denmark::Denmark
:C:copenhagen::Copenhagen
:C:djibouti::Djibouti
:C:djibouti city::Djibouti City
:C:dominica::Dominica
:C:roseau::Roseau
:C:dominican republic::Dominican Republic
:C:santo domingo::Santo Domingo
:C:ecuador::Ecuador
:C:quito::Quito
:C:guayaquil::Guayaquil
:C:egypt::Egypt
:C:cairo::Cairo
:C:alexandria::Alexandria
:C:el salvador::El Salvador
:C:san salvador::San Salvador
...
:C:paris::Paris
:C:lyon::Lyon
:C:marseille::Marseille
:C:gabon::Gabon
:C:libreville::Libreville
:C:gambia::Gambia
:C:banjul::Banjul
:C:georgia::Georgia
:C:tbilisi::Tbilisi
...
:C:athens::Athens
:C:grenada::Grenada
:C:saint george's::Saint George's
:C:guatemala::Guatemala
:C:guatemala city::Guatemala City
:C:guinea::Guinea
:C:conakry::Conakry
:C:guinea-bissau::Guinea-Bissau
:C:bissau::Bissau
:C:guyana::Guyana
:C:georgetown::Georgetown
:C:haiti::Haiti
:C:port-au-prince::Port-au-Prince
:C:honduras::Honduras
:C:tegucigalpa::Tegucigalpa
:C:hungary::Hungary
:C:budapest::Budapest
:C:iceland::Iceland
:C:reykjavik::Reykjavík
:C:india::India
:C:new delhi::New Delhi
:C:mumbai::Mumbai
:C:bengaluru::Bengaluru
:C:chennai::Chennai
...
:C:pristina::Pristina
:C:kuwait::Kuwait
:C:kuwait city::Kuwait City
:C:kyrgyzstan::Kyrgyzstan
:C:bishkek::Bishkek
:C:laos::Laos
:C:vientiane::Vientiane
:C:latvia::Latvia
:C:riga::Riga
:C:lebanon::Lebanon
:C:beirut::Beirut
...
:C:luxembourg city::Luxembourg City
:C:madagascar::Madagascar
:C:antananarivo::Antananarivo
:C:malawi::Malawi
:C:lilongwe::Lilongwe
:C:malaysia::Malaysia
:C:kuala lumpur::Kuala Lumpur
:C:putrajaya::Putrajaya
:C:maldives::Maldives
:C:malé::Malé
:C:mali::Mali
:C:bamako::Bamako
...
:C:nouakchott::Nouakchott
:C:mauritius::Mauritius
:C:port louis::Port Louis
:C:mexico::Mexico
:C:mexico city::Mexico City
:C:guadalajara::Guadalajara
:C:monterrey::Monterrey
:C:micronesia::Micronesia
:C:palikir::Palikir
:C:moldova::Moldova
:C:chisinau::Chișinău
:C:monaco::Monaco
:C:monaco::Monaco
:C:mongolia::Mongolia
:C:ulaanbaatar::Ulaanbaatar
:C:montenegro::Montenegro
:C:podgorica::Podgorica
:C:morocco::Morocco
:C:rabat::Rabat
:C:casablanca::Casablanca
:C:mozambique::Mozambique
:C:maputo::Maputo
:C:myanmar::Myanmar
:C:naypyidaw::Naypyidaw
:C:yangon::Yangon
:C:namibia::Namibia
:C:windhoek::Windhoek
:C:nauru::Nauru
...
:C:abuja::Abuja
:C:lagos::Lagos
:C:north korea::North Korea
:C:pyongyang::Pyongyang
:C:north macedonia::North Macedonia
:C:skopje::Skopje
:C:norway::Norway
:C:oslo::Oslo
:C:oman::Oman
:C:muscat::Muscat
:C:pakistan::Pakistan
:C:islamabad::Islamabad
:C:karachi::Karachi
:C:lahore::Lahore
:C:palau::Palau
:C:ngerulmud::Ngerulmud
:C:panama::Panama
:C:panama city::Panama City
:C:papua new guinea::Papua New Guinea
:C:port moresby::Port Moresby
:C:paraguay::Paraguay
:C:asuncion::Asunción
:C:peru::Peru
:C:lima::Lima
...
:C:singapore::Singapore
:C:slovakia::Slovakia
:C:bratislava::Bratislava
:C:slovenia::Slovenia
:C:ljubljana::Ljubljana
:C:solomon islands::Solomon Islands
:C:honiara::Honiara
:C:somalia::Somalia
:C:mogadishu::Mogadishu
:C:south africa::South Africa
:C:pretoria::Pretoria
:C:cape town::Cape Town
:C:bloemfontein::Bloemfontein
:C:south korea::South Korea
:C:seoul::Seoul
:C:south sudan::South Sudan
:C:juba::Juba
:C:spain::Spain
:C:madrid::Madrid
:C:barcelona::Barcelona
:C:sri lanka::Sri Lanka
:C:sri jayawardenepura kotte::Sri Jayawardenepura Kotte
:C:colombo::Colombo
:C:sudan::Sudan
:C:khartoum::Khartoum
:C:suriname::Suriname
:C:paramaribo::Paramaribo
:C:sweden::Sweden
...
:C:lome::Lomé
:C:tonga::Tonga
:C:nuku'alofa::Nukuʻalofa
:C:trinidad and tobago::Trinidad and Tobago
:C:port of spain::Port of Spain
:C:tunisia::Tunisia
:C:tunis::Tunis
:C:turkey::Turkey
:C:ankara::Ankara
...
:C:washington dc::Washington, DC
:C:new york::New York
:C:los angeles::Los Angeles
:C:chicago::Chicago
:C:usa::USA
:C:uruguay::Uruguay
:C:montevideo::Montevideo
:C:uzbekistan::Uzbekistan
:C:tashkent::Tashkent
:C:vanuatu::Vanuatu
:C:port vila::Port Vila
:C:vatican city::Vatican City
:C:vatican city::Vatican City
:C:venezuela::Venezuela
:C:caracas::Caracas
:C:vietnam::Vietnam
:C:hanoi::Hà Nội
:C:ho chi minh city::Ho Chi Minh City
:C:yemen::Yemen
:C:sana'a::Sana'a
:C:zambia::Zambia
:C:lusaka::Lusaka
